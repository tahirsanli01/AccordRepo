...
namespace Accord.MachineLearning.DecisionTrees
{
    ...
    public class DecisionTree : MulticlassClassifierBase, IEnumerable<DecisionNode>
    {
        ...
        {
            // Dinamik assembly ve modül oluşturma
            AssemblyBuilder da = AssemblyBuilder.DefineDynamicAssembly(new AssemblyName(assemblyName), AssemblyBuilderAccess.RunAndCollect);
            ModuleBuilder dm = da.DefineDynamicModule(moduleName + ".dll");
            TypeBuilder dt = dm.DefineType(className, TypeAttributes.Public);

            // Dinamik metod oluşturma
            MethodBuilder method = dt.DefineMethod("Compute", MethodAttributes.Public | MethodAttributes.Static, typeof(int), new Type[] { typeof(double[]) });

            // Expression tanımlama
            Expression<Func<double[], int>> expression = (arr) => arr.Length;

            // Expression'ı derleme
            var compiled = expression.Compile();

            // ILGenerator kullanarak expression'ı metoda yerleştirme
            ILGenerator ilGenerator = method.GetILGenerator();
            ilGenerator.Emit(OpCodes.Ldarg_0);
            ilGenerator.Emit(OpCodes.Call, compiled.Method);
            ilGenerator.Emit(OpCodes.Ret);

            // Dinamik tipi oluşturma
            dt.CreateType();

            // Assembly'i kaydetme
            da.Save(assemblyName + ".dll");
        }
